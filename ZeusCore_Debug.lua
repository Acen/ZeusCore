...
function self.Console.add(item)
    ...
    if(self.echo) then
        d('[Zeus.Debug:'.. item.type .. '] ' .. message)
    end
...
function self.Console.parse(messageTable)
    ...
    if(argCount == nil) then
        ...
        self.Console.error("Zeus.Debug.Console.parse: messageTable.n is nil, seems like parse is being misused.\nThis is made for catching the `arg` magic arugment off of a function's '...' field")
    end
    if(argCount == 1) then
        ...
        error("Zeus.Debug.Console.parse currently only accepts one argument.")
    end
end
...
function self.Console.Elements()
    GUI:BeginChild("##ZeusDebug_Console", 0, 0, false)
        GUI:BeginChild("##ZeusDebug_Console_History", 0, 0, true)
...
function self.onInitialize()
    ml_gui.ui_mgr:AddMember({ id = "FFXIVMINION##MENU_ZeusDebug", name = "ZeusDebug", onClick = function() self.options.open = not self.options.open end, tooltip = "Zeus Debug"},"FFXIVMINION##MENU_HEADER")
end

function self.onDraw(_, ...)
    if(self.options.open) then
        GUI:SetNextWindowSize(500, 500, GUI.SetCond_FirstUseEver)
        self.options.visible, self.options.open = GUI:Begin("Zeus Debug", self.options.open, GUI.WindowFlags_MenuBar)
...
RegisterEventHandler("Module.Initalize", self.onInitialize, "Zeus.Debug.onInitialize")
RegisterEventHandler("Gameloop.Draw", self.onDraw, "Zeus.Debug.onDraw")