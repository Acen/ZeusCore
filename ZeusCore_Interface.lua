Zeus.Interface = {
    moduleName = "Zeus.Interface",
    open = true,
    visible = true,
    currentTab = 1,
}
local self = Zeus.Interface
local core = Zeus.Core
self.internal = {
    currentACR = nil,
    settings = {}
}
...
local function getSetting(module, key, default)
    if(Zeus.Settings ~= nil) then
        if(Zeus.Settings.getSetting ~= nil) then
            return Zeus.Settings.getSetting(module, key, default)
        end
    end
end
...
local function setSetting(module, key, value)
    if(Zeus.Settings ~= nil) then
        if(Zeus.Settings.setSetting ~= nil) then
            Zeus.Settings.setSetting(module, key, value)
        end
    end
end
...
function self.parseOptionSlider(slider, moduleName)
    ...
    local changeSlider
    value, changeSlider = GUI:SliderInt(value .. '%', value, slider.min, slider.max)
    if(changeSlider) then
        dispatch("Zeus.Settings.SaveSetting", {moduleName, slider.id, value})
        --log("Changed [" .. slider.id .. "] to " .. tostring(self.internal.settings[moduleName][slider.id]))
        --setSetting(moduleName, slider.id, self.internal.settings[moduleName][slider.id])
    end
...
function self.onDraw(_, ...)
    if(self.open) then
        ...
        self.visible, self.open = GUI:Begin("Zeus Core", self.open, GUI.WindowFlags_NoCollapse)

                GUI:BeginChild("##ZeusCore_Tabs", 200, -1, false, GUI.WindowFlags_NoResize)
                    ...
                    for k, v in ipairs(self.tabs) do
                        if(v.image ~= nil) then
                            ...
                            if(GUI:ImageButton("##ZeusCore_" .. v.name .. "Button", v.image.path, v.image.width, v.image.height, 0, 0, 1, 1, 0, 0, 0, 0, 0)) then
                ...
                GUI:BeginChild("##ZeusCore_Options", -1, -1, false, GUI.WindowFlags_AlwaysAutoResize)
                    if(self.currentTab == 1) then
                        GUI:Text("Welcome to ZeusCore ~") GUI:SameLine()
                    end
...
function self.loadACR(_, ...)
    ...
    if(decodedTab.moduleName ~= nil) then
        log('loadACR: ' .. decodedTab.moduleName)
    else
        log('loadacr - no modulename')
    end

    table.insert(self.tabs, decodedTab)
end

RegisterEventHandler("Module.Initalize", self.onInitialize, "Zeus.Interface.onInitialize")
RegisterEventHandler("Gameloop.Draw", self.onDraw, "Zeus.Interface.onDraw")

-- Custom Events
RegisterEventHandler("Zeus.Events.LoadACR", self.loadACR, "Zeus.Interface.onACRAvailable")
RegisterEventHandler("Zeus.Events.ACRChanged", self.onACRChanged, "Zeus.Interface.onACRChanged")
